...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $conn = new mysqli("localhost", "root", "", "users_db"); // update this line
    ...
    $sql = "INSERT INTO users (first_name, last_name, email, password, profile_image, course, year, block, role)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssss", $first_name, $last_name, $email, $password, $profile_image, $course, $year, $block, $role);
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <form method="POST" action="" class="bg-white p-8 rounded-lg shadow-md w-full max-w-lg space-y-4">
        ...
        <button type="submit" class="w-full bg-green-700 text-white font-semibold py-2 rounded hover:bg-green-800 transition">Register</button>
    </form>
