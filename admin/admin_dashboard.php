...
// Fetch user details (single query to get everything needed)
$stmt = $conn->prepare("SELECT role, status, first_name, last_name, profile_image FROM users WHERE email = ?");
$stmt->bind_param('s', $email);
$stmt->execute();
$stmt->bind_result($role, $status, $firstName, $lastName, $profileImage);
$stmt->fetch();
$stmt->close();
...
// Count pending event registrations (for notification badge, etc.)
$pendingCount = 0;
...
<body class="flex h-screen text-gray-800 font-sans" style="background-color: #FAF5EE;" x-data="{ sidebarOpen: false }">
  ...
  <div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <nav class="px-6 flex flex-col space-y-3 mt-6">
      <a href="admin_dashboard.php" class="block px-4 py-2 rounded font-semibold"
        style="background-color: #1D503A; color: #FAF5EE;">Home</a>
    ...
    <main class="flex-1 overflow-y-auto p-6">
      <div class="max-w-4xl mx-auto">
        <div class="text-center mb-6">
          ...
          <p class="text-lg text-[#4A5D4C] border-b-2 pb-2" style="border-color: #1D503A;">Manage your events</p>
        </div>

        <!-- Event Creation -->
        <div class="p-6 rounded-xl shadow border" style="background-color: #FAF5EE; border-color: #1D503A;">
          <h3 class="text-2xl font-semibold mb-2 text-center" style="color: #1D503A;">Create Event</h3>

          <!-- Reserve space for message -->
          <p class="text-sm text-center mb-4 min-h-[1.25rem]"
            style="color: <?php
                          if (isset($_SESSION['event_message'])) {
                            echo ($_SESSION['event_message_type'] ?? 'error') === 'success' ? '#16a34a' /*green-600*/ : '#dc2626'; /*red-600*/
                          } else {
                            echo 'transparent';
                          }
                          ?>;">
          ...
          <form action="create-event.php" method="POST" class="space-y-6">
            <input type="text" name="title" placeholder="Event Title" required
              class="w-full px-4 py-3 border rounded-lg"
              style="border-color: #1D503A; color: #1D503A;" />
            <textarea name="description" placeholder="Description" required
              class="w-full px-4 py-3 border rounded-lg resize-y"
              style="border-color: #1D503A; color: #1D503A;"></textarea>
            <input type="date" name="event_date" required
              class="w-full px-4 py-3 border rounded-lg"
              style="border-color: #1D503A; color: #1D503A;" />
            <button type="submit" class="w-full py-3 rounded-lg btn-primary font-medium">
              Add Event
            </button>
          </form>
        </div>
