...
// Get profile image for user if logged in
...
// Check for registration success message in URL query string
...
<section
  ...
  <div class="max-w-4xl mx-auto">
    <div class="text-center mb-6">
      <h2 class="text-xl font-semibold text-[#1D503A]">Available Events</h2>
      <p class="text-lg text-[#4A5D4C] border-b-2 pb-2 border-[#1D503A]">
        View and register to an existing event.
      </p>
    </div>
  </div>

  <!-- Success Message Modal -->
  <!-- ... same modal for registration success ... -->
        ...
        <table class="min-w-full text-sm text-left text-gray-700">
          <tbody>
            <?php while ($event = $events->fetch_assoc()): ?>
              <?php
              // Same code for getting registration status
              $statusMessage = '';
  ...
  <div
    ...
    <div
      ...
      <div class="mb-6">
        ...
        <p class="text-lg font-medium text-[#4A5D4C]" x-text="selectedEvent.title"></p>
      </div>

      <div class="mb-6">
        <label class="block text-sm font-semibold text-[#1D503A] mb-1">Date:</label>
        <p class="text-lg font-medium text-[#4A5D4C]" x-text="selectedEvent.date"></p>
      </div>

      <div class="mb-6">
        <label class="block text-sm font-semibold text-[#1D503A] mb-1">Description:</label>
        <p class="text-lg font-medium text-[#4A5D4C]" x-text="selectedEvent.description"></p>
      </div>

      <button
        @click="showModal = false"
        class="block mx-auto px-8 py-2 mt-6 rounded-full text-lg font-bold bg-[#1D503A] text-[#E5E1DB] hover:bg-[#17412F] focus:outline-none focus:ring-4 focus:ring-[#1D503A]/50">
        Close
      </button>
    </div>
  </div>
