...
<section
  ...
  <h2 class="text-2xl text-center border-b-2 font-semibold mb-6 text-[#1D503A]" style="border-color: #1D503A;">Available Events</h2>
  ...
  <div
    ...
    <div class="relative bg-green-100 text-green-800 px-6 py-4 rounded shadow-lg max-w-md text-center font-semibold">
      ...
      <button @click="showSuccessModal = false" class="absolute top-2 right-2 text-green-800 hover:text-green-900 font-bold text-xl leading-none" aria-label="Close modal">&times;</button>
    </div>
  </div>

  <?php if ($events && $events->num_rows > 0): ?>
    <!-- Table Only If Events Exist -->
        ...
        <tbody>
          <?php while ($event = $events->fetch_assoc()): ?>
            ...
            <tr class="border-b border-gray-200 hover:bg-gray-50 transition duration-200 text-sm">
              <td class="px-4 py-3 sm:px-6 sm:py-4 font-medium text-gray-900 whitespace-nowrap"><?= htmlspecialchars($event['title']) ?></td>
              <td class="px-4 py-3 sm:px-6 sm:py-4 text-gray-700 whitespace-nowrap"><?= htmlspecialchars($event['event_date']) ?></td>
              <td class="px-4 py-3 sm:px-6 sm:py-4 text-gray-600 truncate max-w-sm"><?= htmlspecialchars($event['description']) ?></td>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center gap-2">
                  <?php if (!$statusMessage): ?>
                    <button
                      ...
                      class="w-full sm:w-auto px-3 py-1.5 text-xs font-semibold rounded-lg shadow hover:shadow-md"
  ...
  <?php else: ?>
    <!-- No Events Message -->
    <div class="text-center text-[#1D503A] select-none mt-12" role="alert" aria-live="polite"
